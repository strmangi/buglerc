            ...
            <div class="row">
              <div class="col-sm-4">
                <div class="form-group">
                  ...
                  <input type="text" name="reporting_time" class="form-control reporting_time" id="reporting_time" data-parsley-required="true" data-parsley-trigger="change" value="{{ date('d-m-Y', strtotime($trips[0]->trip_start_date)).$details->report_time }}">
                </div>
              </div>
              ...
              <div class="col-sm-4">
                <div class="form-group">
                  ...
                  <input type="text" name="departure_time" class="form-control ending_time" id="ending_time" data-parsley-required="true" data-parsley-trigger="change" value="{{ date('D d-m-Y', strtotime($trips[0]->trip_start_date)).$trips[0]->pick_up_time }}">
            ...
            @for($i = 1; $i <= $trip->no_of_coach; $i++)
              ...
              <div class="row">
                  ...
                  <div class="form-group">
                    ...
                    <select class="form-control selectCoach" name="coach[]" required="" id="coach_{{$j}}">
                      ...
                      @foreach($coaches as $coach)
                       <?php $data = DB::table("coach_driver_trip")->where('trip_id', $trips[0]->inq_id)->get();

                           echo $coach_id = isset($data[0]->coach_id) ? $data[0]->coach_id : '';
                ...
                ?>
                      ...
                      <option value="{{$coach->id}}"  {{ $autslct }} >{{$coach->coach_name.' || '.$coach->registration_no}}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
      ...
      if (reportingTime && endingTime && trip_end_time) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            $(".driver").empty();
             $(".driver").append("<option value=''>-Select-</option>");
            for (var i = 0; i < len; i++) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            $(".selectCoach").empty();
             $(".selectCoach").append("<option value=''>-Select-</option>");
            for (var i = 0; i < len; i++) {
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $('#validate_form').on('submit', function(event) {
      ...
      //var appBaseUrl = location.protocol + "//" + location.hostname + (location.port && ":" + location.port) + "/";
